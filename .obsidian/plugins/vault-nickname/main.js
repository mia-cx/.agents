...
var VaultNicknamePlugin = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    this.registerEvent(
      this.app.vault.on("rename", this.vaultItemRenamedCallback),
    );
    this.registerEvent(
      this.app.workspace.on(
        "active-leaf-change",
        this.activeLeafChangeCallback,
      ),
    );
  }
  onunload() {
  ...
  onLayoutReady() {
    ...
    if (!originalDesktopVaultSwitcherElement) {
      ...
      this.desktopVaultSwitcherElement =
        originalDesktopVaultSwitcherElement.createDiv(
  ...
  onVaultSwitcherClicked(event2) {
    ...
    for (let vaultKey in vaults) {
      ...
      let vaultPluginSettingsFilePath = (0, import_obsidian.normalizePath)(
        [vault.path, VAULT_LOCAL_SHARED_SETTINGS_FILE_PATH].join(
          PATH_SEPARATOR,
        ),
      );
      if (
        import_obsidian.Platform.isLinux &&
        !vaultPluginSettingsFilePath.startsWith(PATH_SEPARATOR)
      ) {
        vaultPluginSettingsFilePath =
          PATH_SEPARATOR + vaultPluginSettingsFilePath;
      }
      if (this.filePathExistsSync(vaultPluginSettingsFilePath)) {
        const vaultPluginSettingsJson = this.readUtf8FileSync(
          vaultPluginSettingsFilePath,
        );
  ...
  async onVaultSwitcherContextMenu() {
    ...
    const showInFolderText = import_obsidian.Platform.isMacOS
      ? window.OBSIDIAN_DEFAULT_I18N.plugins.openWithDefaultApp
          .actionShowInFolderMac
      : window.OBSIDIAN_DEFAULT_I18N.plugins.openWithDefaultApp
          .actionShowInFolder;
    menu.addItem((item) =>
      item
  ...
  async openVaultNicknameSettings() {
    ...
    const settingsMenu = await this.waitForSelector(
    ...
    const anyTab = await this.waitForSelector(
      window.activeDocument,
    ...
    if (!anyTab) {
      console.error(
        "Timeout while waiting for a settings menu tab to be found.",
      );
      return;
    }
    const settingsTabs = settingsMenu.querySelectorAll(
      ".vertical-tab-nav-item",
    );
      ...
      new import_obsidian.Setting(containerEl)
        ...
        .addDropdown((dropdownComponent) => {
          ...
          dropdownComponent.addOption(
            "override-app-title:vault-first",
            "Vault name first",
          );
          dropdownComponent.addOption(
            "override-app-title:file-first",
            "File name first",
          );
