...
import type { AgentConfig, ConstraintSet, MeetingMode, ParsedBrief } from "./types.js";
...
export interface MessagingMeetingCallbacks extends Pick<MeetingCallbacks, "onStatus" | "onConfirmRoster" | "signal"> {}
...
export async function runFreeformMessagingMeeting(
  ...
  try {
    ...
    const rosterDecision = await callbacks.onConfirmRoster(rosterAgents, nonCeo, rosterRationale);
    ...
    const isAbort = err.message === "Subagent was aborted" || callbacks.signal?.aborted;
    if (isAbort) {
      callbacks.onStatus("Meeting aborted. Saving partial state...");
    } else {
      callbacks.onStatus(`Meeting error: ${err.message}. Saving partial state...`);
    }
    ...
    const resolvedCount = resolveAllActiveThreads(threadState, reason, `Meeting ${disposition}: ${err.message ?? "unknown error"}`);
    ...
    const messagingLog = serializeToMessagingLog(
      threadState, meetingId, brief.filePath, mode, constraintsName,
      ["ceo"], startedAt.toISOString(), disposition,
    );
    ...
    return {
      ...
      visualPaths,
      disposition,
      abortReason: err?.message ?? "Meeting aborted.",
      briefTitle: brief.title,
      mode,
      totalCost: tracker.totalCost,
      elapsedMinutes: tracker.elapsedMinutes,
      roster: messagingLog.roster,
    };
  }
}
...
export async function runStructuredMessagingMeeting(
  ...
  try {
    ...
    const rosterDecision = await callbacks.onConfirmRoster(rosterAgents, nonCeo, rosterRationale);
    if (rosterDecision.action === "cancel") {
      callbacks.onStatus("Structured meeting cancelled during roster review.");
      throw new Error("Subagent was aborted");
    }
    if (rosterDecision.action === "reject") {
      rosterAgents = [...nonCeo];
    ...
    if (isAbort) {
      ...
      callbacks.onStatus(`Structured meeting error: ${err.message}. Saving partial state...`);
    }
