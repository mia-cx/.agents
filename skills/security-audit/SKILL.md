...
Full-codebase security assessment. Unlike `review-pr` (which scopes the Security Reviewer agent to a single PR diff), this skill points it at the entire repository and supplements with automated scanning.
...
**Dependency audit** (delegate to the `dep-audit` skill for full treatment, but grab the summary):
...
- **Dependencies**: [N critical, N high, N moderate CVEs — see dep-audit for full treatment]
...
- [ ] Run `dep-audit` skill for full dependency upgrade plan
...
- **Delegate dependency work to `dep-audit`.** This skill focuses on code and config, not package versions.
